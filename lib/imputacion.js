...
import { soloDigitos, conPnp, nombreCompletoVisible, nombreParaSello, normalizarMencionInvestigado, splitApellidosNombres, fechaLarga, fechaCorta, tokens, buscarOficialConstato } from "./utils.js";
...
export function construirDatosImputacion(caso, efectivos) {
  ...
  const oficialSeparado = splitApellidosNombres(oficial.apellidos_nombres);
  const hechoBase = normalizarMencionInvestigado(caso.descripcion_hecho.trim(), caso.apellidos, caso.nombres);
  const puesto = String(caso.puesto_rol || "").trim();
  const hechoConRol = puesto ? `${hechoBase}\n\nConforme al rol de servicio del día, al investigado se le asignó: ${puesto}.` : hechoBase;

  return {
    investigado_completo: investigadoCompleto,
    unidad_investigado: caso.unidad_investigado || "DIVOPUS 3-CPNP VENTANILLA.",
    descripcion_hecho: hechoConRol,
    ...
    // Se usa tanto en el texto del documento como en el sello: nombres con
    // formato título y apellidos en MAYÚSCULAS.
    superior_completo: `${conPnp(oficial.grado)} ${nombreParaSello(oficialSeparado.apellidos, oficialSeparado.nombres)}.`.replace(/\s+/g, " ").trim(),
    ...
    oficial_nombre_completo: nombreParaSello(oficialSeparado.apellidos, oficialSeparado.nombres),
...
export async function generarImputacionDocx(caso, efectivos) {
  ...
  const nombreArchivo = `IMPUTACION LEVE - ${(caso.grado || "").trim()} ${nombreCompletoVisible(caso.apellidos, caso.nombres)}.docx`.replace(/\s+/g, " ").trim();
