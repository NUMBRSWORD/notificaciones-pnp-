...
export function limpiarNombreVisible(s) {
  ...
  if (tieneComa) {
    ...
    if (palabras.length <= 2) return tituloPorPalabra(limpio);
    apellidos = palabras.slice(0, 2).join(" ");
    nombres = palabras.slice(2).join(" ");
  }
  if (!nombres) return tituloPorPalabra(apellidos);
  return `${tituloPorPalabra(nombres)} ${tituloPorPalabra(apellidos)}`.trim();
}

// Mismo formato para los campos separados de un investigado: nombres primero
// y apellidos después. El parámetro separador se mantiene por compatibilidad.
export function nombreCompletoVisible(apellidos, nombres, separador = " ") {
  const apTitulo = tituloPorPalabra(apellidos);
  const noTitulo = tituloPorPalabra(nombres);
  return noTitulo ? `${noTitulo}${separador}${apTitulo}`.trim() : apTitulo;
}
