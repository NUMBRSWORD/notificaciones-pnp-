...
export function construirDatosOrdenSancion(caso, efectivos, seleccion) {
  ...
  return {
    ...
    // El sello firma con nombre y apellidos en formato título, no TODO-MAYÚSCULAS.
    signer_nombre: nombreCompletoVisible(superiorSplit.apellidos, superiorSplit.nombres),
...
export async function generarOrdenSancionDocx(caso, efectivos, seleccion) {
  ...
  const nombreArchivo = `ORDEN DE SANCION - ${(caso.grado || "").trim()} ${(caso.nombres || "").trim()} ${(caso.apellidos || "").trim()}.docx`.replace(/\s+/g, " ").trim();
