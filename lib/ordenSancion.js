...
// del hecho en texto libre que ya escribió el oficial, sin repetir el texto
// normativo de la infracción que la plantilla ya muestra por separado.
//
...
export function puedeGenerarOrdenSancion(caso, efectivos) {
  ...
  if (!caso.investigado_cip && !buscarInvestigado(caso.apellidos, caso.nombres, efectivos)) return false;
  return true;
}
...
export function construirDatosOrdenSancion(caso, efectivos, seleccion) {
  ...
  const investigadoCip = caso.investigado_cip || investigadoEf?.cip;
  if (!investigadoCip) {
    throw new Error("No se pudo ubicar el CIP del investigado. En casos antiguos, verifique el padrón antes de generar la orden.");
  }
  ...
  const descargoOriginal = (seleccion.descargoTexto || "").trim() ||
  ...
  const descargoTexto = normalizarMencionInvestigado(descargoOriginal, caso.apellidos, caso.nombres);
  const descripcionBase = normalizarMencionInvestigado(caso.descripcion_hecho || "", caso.apellidos, caso.nombres);
  const puesto = String(caso.puesto_rol || "").trim();
  const descripcionHecho = puesto ? `${descripcionBase}\n\nConforme al rol de servicio del día, al investigado se le asignó: ${puesto}.` : descripcionBase;
  const analisisTexto = normalizarMencionInvestigado(seleccion.analisisTexto.trim(), caso.apellidos, caso.nombres);

  // La plantilla muestra el código y la descripción normativa en
  // `codigo_texto`. Aquí debe ir exclusivamente el hecho concreto que ya se
  // redactó y usó en la imputación, sin añadir ni duplicar la infracción.
  const hechoCompleto = descripcionHecho;

  const decisionTexto =
    `Se resuelve SANCIONAR al ${investigadoCompleto}, CIP N° ${investigadoCip}, perteneciente a la comisaría PNP Ventanilla, ` +
  ...
  return {
    ...
    codigo_texto: `${codigoConGuion} (${infraccion.infraccion.replace(/\.\s*$/, "")}).`,
    ...
    // El sello conserva los nombres legibles y resalta los apellidos en mayúsculas.
...
export async function generarOrdenSancionDocx(caso, efectivos, seleccion) {
  ...
  const nombreArchivo = `ORDEN DE SANCION - ${(caso.grado || "").trim()} ${nombreCompletoVisible(caso.apellidos, caso.nombres)}.docx`.replace(/\s+/g, " ").trim();
