...
import { splitApellidosNombres, plazoDescargoVencido, fechaLimiteDescargo } from "./utils.js";
...
export function construirDatosActaNoDescargo(caso, efectivos) {
  ...
  return {
    ...
    investigado_apellidos: caso.apellidos || "",
    investigado_nombres: caso.nombres || "",
...
export async function generarActaNoDescargoDocx(caso, efectivos) {
  ...
  const nombreArchivo = `ACTA NO DESCARGO - ${(caso.grado || "").trim()} ${(caso.apellidos || "").trim()} ${(caso.nombres || "").trim()}.docx`.replace(/\s+/g, " ").trim();
